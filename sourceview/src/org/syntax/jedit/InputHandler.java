    ...
    public static class end implements ActionListener
    {
        ...
        public void actionPerformed(ActionEvent evt)
        {
            ...
            if (caret == lastDocument) {
                ...
                if (!Boolean.TRUE.equals(textArea.getClientProperty(SMART_HOME_END_PROPERTY))) {
                    caret = lastOfLine;
                } else {     
                    int newCaret = lastOfLine;
                    String text = textArea.getLineText(textArea.getCaretLine());
                    int index = text.length()-1;
                    while (index >= 0 && TextUtilities.isWhitespace(text.charAt(index--))) {
                        --newCaret;
                    }
                    //if we are already at the last non-whitespace char
        ...
        public void actionPerformed(ActionEvent evt)
        {
            ...
            if (caret == 0) {
                ...
                if (!Boolean.TRUE.equals(textArea.getClientProperty(SMART_HOME_END_PROPERTY))) {
                    ...
                    int newCaret = firstOfLine;
                    String text = textArea.getLineText(textArea.getCaretLine());
                    int index = 0;
                    while (index < text.length() && TextUtilities.isWhitespace(text.charAt(index++))) {
                        ++newCaret;
                    }
                    //if we are already at the last non-whitespace char
                    //then go to end of line.
